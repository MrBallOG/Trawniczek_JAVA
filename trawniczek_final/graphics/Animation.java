...
import io.Output;
import positioning.UpdateSprinklers;
...
public class Animation extends JPanel {
	...
	private void calculate() {
		us = new UpdateSprinklers(lawn, num, period, set_rebounds, q);
		
		SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>(){
			@Override
			protected Void doInBackground() throws Exception {
				calculate = new Thread(us);
				calculate.start();
				calculate.join();
				return null;
			}
		...
		public void run() {
			while(us.getRunning()) {
				try {
					...
					us.animationGotInterrupted();
				}
				repaint();
			}
			paintBitmap();
			out = new Output(us.getSprlist());
			out.writeToFile();
